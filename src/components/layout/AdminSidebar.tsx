...
import { LayoutDashboard, Users, Sprout, Package, ShoppingBag, Tag, Flag, Settings, LogOut, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export const AdminSidebar: React.FC<AdminSidebarProps> = ({ isCollapsed, onToggle, isMobile, onClose }) => {
  ...
  return (
    <aside className={cn(
      ...
      <div className={cn(
        ...
        <div className="flex items-center space-x-3 overflow-hidden" onClick={() => handleNavClick('/admin')}>
      ...
      <nav className="flex-1 overflow-y-auto py-8 px-4 space-y-2 scrollbar-hide">
        {(isMobile || !isCollapsed) && (
          <div className="px-4 mb-4 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Admin Menu</div>
        )}
        {navItems.map((item) => {
