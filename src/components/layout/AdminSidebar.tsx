...
import { LayoutDashboard, Users, Sprout, Package, ShoppingBag, Tag, Flag, Settings, Leaf, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <aside className={cn("hidden lg:flex flex-col bg-neutral-900 text-white h-screen fixed top-0 left-0 z-50 shadow-2xl transition-all duration-300", isCollapsed ? "w-20" : "w-72")}>
      ...
      <nav className="flex-1 overflow-y-auto py-8 px-4 space-y-2 scrollbar-hide">
        {!isCollapsed && <div className="px-4 mb-4 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Management</div>}
        {navItems.map((item) => {
      ...
      <div className={cn("p-4 bg-neutral-950/80 backdrop-blur-md border-t border-white/5 mt-auto", isCollapsed ? "items-center flex flex-col" : "")}>
        <div className={cn("flex items-center mb-4", isCollapsed ? "justify-center px-0" : "px-2")}>
          <div className="h-11 w-11 rounded-full bg-gradient-to-br from-neutral-700 to-neutral-800 flex items-center justify-center text-white font-bold overflow-hidden shadow-md ring-2 ring-white/10 border border-white/5 shrink-0">
            {user?.avatar ? (
